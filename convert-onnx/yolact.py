...
############ NEED TO FIX FOR USING CONVERT ONNX #############
        ...
        # return preds
        if cfg.use_mask_scoring and cfg.use_instance_coeff:
            return bbox, conf, mask, priors, score, inst
        elif cfg.use_mask_scoring and not cfg.use_instance_coeff:
            return bbox, conf, mask, priors, score
        elif not cfg.use_mask_scoring and cfg.use_instance_coeff:
            return bbox, conf, mask, priors, inst
        else:
            return bbox, conf, mask, priors
#############################################################       
...
############ COMMENT @script_method_wrapper FOR USING CONVERT ONNX #############
...
############## REPLACE DETECT LAYER BEFORE CONVERT ONNX #################
        # # # Original source code
...
############ NEED TO FIX FOR USING CONVERT ONNX #############
        ...
        if cfg.use_mask_scoring and cfg.use_instance_coeff:
            score = torch.cat(score, -2)
            inst = torch.cat(inst, -2)
        elif cfg.use_mask_scoring and not cfg.use_instance_coeff:
            score = torch.cat(score, -2)
        elif not cfg.use_mask_scoring and cfg.use_instance_coeff:
            inst = torch.cat(inst, -2)
        ...
        if cfg.use_focal_loss:
            if cfg.use_sigmoid_focal_loss:
                ...
                objectness = torch.sigmoid(conf[:, :, 0])
                conf[:, :, 1:] = objectness[:, :, None] * F.softmax(conf[:, :, 1:], -1)
                conf[:, :, 0 ] = 1 - objectness
            else:
            ...
            if cfg.use_objectness_score:
                ...
                conf[:, :, 1:] = (objectness > 0.10)[..., None] \
                    * F.softmax(conf[:, :, 1:], dim=-1)
        ...
        mask = mask.view(s)
        # _, *s = priors.shape
        # priors = priors.view(s)
        _, *s = proto_out.shape
        proto_out = proto_out.view(s)

        return loc, conf, mask, priors, proto_out
############################################################# 
...
if __name__ == '__main__':
    ...
    net = Yolact(trained_model, device="cpu")
